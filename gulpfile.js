...
/!**
 *============================================
* @task : script병합, 압축, min 파일 생성
 *============================================
 *!/*/


// gulp.tash(name, deps, func)
...
// func는 실제 수행할 업무프로세르르 정의하는 function
gulp.task('default', ['concat:js','watch'], function() { //gulp default는 gulp명령어만을 타이핑하여 실행했을 때 기본으로 실행되는 task다.
    console.log('success!');
})
gulp.task('concat:js',function() {
    return gulp.src('src/js/*.js') //해당 task의 대상이 되는 파일들을 지정해준다. (* 파일을 선택하는 방식은 node-glob의 문법을 따르고 있다.) gulp.src(files)의 메소드 줄은 gulp객체의 src()에 파일이나 파일경로를 배열 혹은 string형태로 작성한다. **, *등의 문자형태인 와일드카드 문자 형태로 표현해줄수있다. 배열을 사용해서 원하는 만큼 여러 개의 파일들과 폴더들을 가져올 수 있다. 와일드카드 문자형태로 병합해줄 때는 파일명의 알파벳 순서대로 병합되므로 스크립트 처리 순서에 이슈가 발생할 수 있다.
        .pipe( concat('combined.js')) //gulp-concat모듈로 호출하여 참조한 concat()  함수
        .pipe(gulp.dest('dist/js')) // gulp의 스트리밍 기능으로 gulp.src에서 대상의 각 파일들을 stram형태로 읽어 다음 플러그인 등으로 연결할 때 사용한다. 즉, pipe 메소드를 사용해 task의 결과물을 function에 전달해줄 수 있다. pipe는 체이닝으로 여러 개의 pipe를 연결해 사용할 수 있다.
